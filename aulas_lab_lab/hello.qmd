...
options(scipen = 999)
birthrateDadosEmPainel <- WDI(country = c("BRA", "AUT", "USA", "ARG", "GBR", "ZAF", "MOZ", "DZA"), 
                              indicator = "SP.DYN.CBRT.IN", 
                              start = "2000", 
                              end = "2022")
...
birthrateCorteTransversal <- WDI(
  country = c("BRA", "AUT", "USA", "ARG", "GBR", "ZAF", "MOZ", "DZA"),
  indicator = 'SP.DYN.CBRT.IN', 
  start = 2021, 
  end = 2021
)
...
## Gráfico Séreie Temporal Austria
...
## Gráfico Séreie Temporal Brasil
...
Os gráficos com **dados em painel** são utilizados para analisar a evolução de uma variável ao longo de um determinado período de tempo em **múltiplas unidades geográficas**, possibilitando a comparação entre elas. Neste caso, o gráfico apresenta a variável **Taxa Bruta de Natalidade** no período de **2000 a 2022**, abrangendo os seguintes países: **Argentina, Áustria, Brasil, Estados Unidos da América, Moçambique, Reino Unido, Argélia e África do Sul**. A representação permite observar e comparar as variações do indicador entre os diferentes países ao longo dos 22 anos analisados.
...
O **gráfico de corte transversal** apresenta os dados de uma variável em um **único período de tempo** (neste caso, um ano específico), considerando **múltiplas unidades geográficas**. Nesta instância, o gráfico exibe a **Taxa Bruta de Natalidade** no ano de **2021**, abrangendo os seguintes países: **Argentina, Áustria, Brasil, Estados Unidos da América, Moçambique, Reino Unido, Argélia e África do Sul**. Essa representação permite a **comparação direta** dos valores do indicador entre os países selecionados no mesmo ponto temporal.
